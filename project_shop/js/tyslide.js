			...
			$.fn.extend({
				tyslide:function(opts){
					...
					$(this).each(function(index,item){//此处循环是因为选择器可能选择了页面上多个轮播
						...
						var w2=boxsount*(prams.controlsW+12);//此宽度是controls的宽度
						...
						//给下方按钮内图标设置数字
//						$(item).find(".controls>li").eq(0).text("1");
//						$(item).find(".controls>li").eq(1).text("2");
//						$(item).find(".controls>li").eq(2).text("3");
//						$(item).find(".controls>li").css({"text-align":"center"});
						//默认设置控制按钮的颜色
